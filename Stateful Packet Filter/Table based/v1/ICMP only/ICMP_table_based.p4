 ...
 *************************************************************************/
    ...
    control IngressDeparser(packet_out pkt,
        ...
        {
            Digest<icmp_id_digest_t>() add_sc2_rule;
            Digest<icmp_id_digest_t>() remove_sc2_rule;
            apply {

                if(ig_dprsr_md.digest_type == 0) {
                    add_sc2_rule.pack({hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.icmp.identifier});
                } else if(ig_dprsr_md.digest_type == 1) {
                    remove_sc2_rule.pack({hdr.ipv4.src_addr, hdr.ipv4.dst_addr, hdr.icmp.identifier});
                }
                pkt.emit(hdr);
            }
        }
